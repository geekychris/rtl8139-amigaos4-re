#!/usr/bin/env bash
# Push rtl8139re.device (and optionally test binaries) to DH1: on the OS4
# guest via the amiga_mcp devbench REST API on localhost:3000.
#
# Usage:
#   ./scripts/deploy.sh                    # just rtl8139re.device
...
push_one "$HERE/build/rtl8139re.device.debug" "DH1:rtl8139re.device"
