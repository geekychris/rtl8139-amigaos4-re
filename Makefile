...
BUILD    = build
DEV      = $(BUILD)/rtl8139re.device
DEV_DBG  = $(BUILD)/rtl8139re.device.debug

# Add every test binary here as it lands. Each corresponds to
# tests/<name>.c, compiled the same way.
TEST_NAMES = testopen offs testtx testrx_diag testmin testg testread testcr testrxpkt testtxtrace testbiotrace testudpsend testopentrace
...
$(DEV_OBJ): $(DEV_SRC) include/version.h include/rtl8139re.h | $(BUILD)
	$(CC) $(CFLAGS) -c $< -o $@

$(DEV_OBJ_DBG): $(DEV_SRC) include/version.h include/rtl8139re.h | $(BUILD)
...
help:
	@echo "rtl8139re.device build system"
	@echo ""
